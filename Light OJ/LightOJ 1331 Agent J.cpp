...
double areaSection(double radius, double a, double b, double c) {
    double angle = acos((a * a + b*b - c*c)/(2*a*b));
    return (radius*radius*angle*0.5);
}
...
int main() {
    ...
    int test, t;
    double r1, r2, r3, a, b, c, triangle, section, actualArea;
    cin >> test;
    for (t = 1; t <= test; ++t) {
        cin >> r1 >> r2 >> r3;
        a = r1 + r2;
        b = r2 + r3;
        c = r3 + r1;
        triangle = sqrt((r1+r2+r3)*r1*r2*r3);
        section = areaSection(r1, a, c, b) + areaSection(r2, a, b, c) +
                  areaSection(r3, c, b, a);
        actualArea = triangle - section;
        printf("Case %d: %.10f\n", t, actualArea);
